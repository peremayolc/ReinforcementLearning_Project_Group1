...
def train_model(env, hyperparameters, target_reward):
    ...
    while frame_number < 800000:
        ...
        with torch.no_grad():
            # Compute next Q values from target network (for target)
            next_Q_values_target = target_net(next_states).max(1, keepdim=True)[0]
            next_Q_values_target[dones] = 0.0 

            # Use online network to select actions for next states (for action selection)
            next_actions = net(next_states).max(1, keepdim=True)[1] 

            # Get Q-values for those actions from the target network
            next_Q_values = target_net(next_states).gather(1, next_actions) 

        # Compute expected Q values using Double DQN
